...
### Changed
- Added compatibility with `sirix/container-resolver` `^1.0` while retaining support for the `^0.2` release line.

## [4.2.0] - 2026-08-12

### Added
- Added `RepositoryFactory` for exposing custom Cycle repositories as container services using their entity-schema mapping.
- Added repository service documentation with read and persist repository examples.

## [4.1.1] - 2026-08-10

### Changed
- Updated `sirix/container-resolver` to `^0.2`.
- Confirmed factory configuration and container-service resolution use the current container-resolver API.

## [4.1.0] - 2026-07-08

### Added
- Added `cycle.collections` configuration for Cycle ORM collection factories.
  It supports a default collection factory and named factories for relation `collection` aliases,
  including built-in Cycle collection factories and custom collection factory implementations.

### Removed
- Removed deprecated `cycle.schema.manual_entity_schema_definition` config key support.
